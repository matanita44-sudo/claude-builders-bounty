...
> **Validated public candidate:** source commit `a550ca867506f34856cf337fbe28083a9cdbaec5`, remote matrix `28,949/0`, exports, soak, deployment, and public Chromium all pass in run `33594396541`. The rebuilt tree separately passes local editor/import and all 21 suites at `40,160/0`, with exact sentinels and zero unexpected engine/script/parse errors; it has no remote/native/deployment proof yet.
...
| Evidence | Result |
|---|---|
| Linux headless Godot main suite | Rebuilt local `2,860 passed, 0 failed`; deployed `a550ca8` remote `2,726/0` |
| Current rebuilt 21-suite matrix | Main `2,860`; backend `100`; upgrades `120`; tutorial `198`; mechanics `3,541`; compiler `15,515`; defender `354`; live defender `212`; travel `685`; integration `4,131`; organs `7,008`; Titan attack specs `855`; meta `111`; audio `684`; story `164`; story presentation `280`; visual `412`; story canon `476`; Story Overlay `34`; Player Presentation `17`; localized layouts `2,403` — all zero failures |
| Current rebuilt local matrix total | `40,160 passed, 0 failed`; editor/import + 21 suites produced exact sentinels with zero unexpected engine `ERROR`, `SCRIPT ERROR`, or parse-error lines. Inventory: 23 scenes / 21 standalone suites / one nested / one soak. The session-local logs were verified but are not retained as repository artifacts; remote proof remains pending. |
| Current local Web export | PASS — release PCK `13,677,600` bytes / SHA-256 `2d90339d64d99c7c0c832f6b55a34e85b421a83be3defa3e3b86d7e5d2dca863`; WebAssembly `39,514,754` bytes. Static/legal validation and exported-PCK headless boot passed, and trailer-only audio was absent from the PCK. Local Linux evidence only; deployment remains pending. |
| Current local bounded soak | `8.003s`; 98 iterations; 10 restarts/Dives; 98 projectile cycles; six saves; 530 queue events; peak 540; all seven movement models; fingerprint `3f5aed099357c66985222681d6b825819e595dbd5ef960e3566003283c37c59a`; zero failures; transaction-recovery self-test PASS. Local structural evidence only. |
| Current deployed candidate CI | PASS for the named automated workflow scope — source commit `a550ca867506f34856cf337fbe28083a9cdbaec5`; Actions run `33594396541`. Validate `100134794261`, Web export `100135597973`, Android debug `100135598026`, unsigned iOS scaffold `100135598028`, long soak `100135598119`, deploy `100141614551`, and public smoke `100141657457` all passed. This does not upgrade native/device/human rows. |
...
| Save isolation | Guard test without the flag exited 1 with `0 passed, 1 failed`; the rebuilt suite with `INFINIDIVE_TEST_ISOLATED=1` plus temporary `XDG_DATA_HOME` exited 0 with `2,860/0`; no production/player profile used |
...
| Paused breach-entry regression | PASS in the rebuilt `2,860/0` main suite — while `BREACH_OPEN` is paused, `_request_dive()` is rejected and the pause overlay remains authoritative; manual resume restores the legal Dive path. |
...
| ORG-005 | Loss-contract catalog | All 12 organs validate unique mechanical variants and unique supported `BossVisual` tokens; the seven-transform/five-disable split is enforced | PASS (7,008-assertion organ suite) |
...
|---|---|---|---|
| Web export artifact | HTML, JavaScript, WASM, PCK presence, excluded tooling/adaptive sources, unresolved-shell token checks, and local HTTP paths | PASS (candidate CI export) | Actions run `33594396541`, Web job `100135597973`, exported commit `a550ca8` and produced CI smoke artifact `9833003906` (4,746,140 bytes / downloaded ZIP SHA-256 `1a6a23bbb8de3896e515cdd7e6f6174115407175b8567a60a1985fa2718981ba`). The rebuilt local export adds notices validation but is not yet remote/deployed evidence. |
...
| Public GitHub Pages URL | Post-deploy HTTP, runtime, and semantic full-path/reload smoke at `https://matanita44-sudo.github.io/claude-builders-bounty/infinidive/` | PASS (current candidate, headless Chromium) | Actions run `33594396541` deployed source commit `a550ca867506f34856cf337fbe28083a9cdbaec5` in job `100141614551`; public-smoke job `100141657457` passed at HTTP 200 with a live 540×960 canvas, complete outside-inside-outside path, Hunter Eye / `homing_eye` degradation, mutation selection, primary-save reload, and zero page/crash/request failures. Artifact `9833689840` is 4,752,918 bytes with downloaded ZIP SHA-256 `b5c2034b1398f852846c476f18c49d758c9fc17836607e06983eeb745a1d7f58`. This is synthetic headless Chromium evidence, not Mobile Safari/Chrome, physical touch, or human feel; it predates the rebuilt hardening delta. |
| Android debug APK | Manifest, permission, architecture, alignment, signature, then install/launch/lifecycle smoke | PARTIAL | Candidate `a550ca8` Android-debug job `100135598026` passed export and structural validation in run `33594396541`; artifact `9832975516` is 32,672,331 bytes / SHA-256 `4791955d62324e399a9ab549712946a9a42a4a23b2e913f21bb9ce6b9e59d77d`. No AAB, emulator/device install, launch, lifecycle, or production-signing result. |
...
| iOS Xcode project | Sanitized unsigned iPhone-targeted scaffold | PARTIAL / BLOCKED | Remote `a550ca8` scaffold evidence remains valid. The rebuilt local scaffold repairs scheme-to-target binding and passes metadata, icon, launch, and privacy validators. Mac/Xcode compile, archive, signature, Simulator/device run, install, and TestFlight remain pending remote push. |
| iPhone 16 Pro Max Simulator | Xcode 26 compile/install/launch/terminate, one boot frame, plus six QA-only 1320×2868 RGB current-bright state captures: Nest, Titan exterior, breach, organ chamber, mutation, and post-organ changed Titan | NOT RUN | Both workflows, scaffold/state/image validators, pinned-action/YAML/Bash checks, and 14 Debug+iOS+CI activation assertions pass locally. No remote Xcode/CoreSimulator job or image artifact has passed; self-tests are not native-app evidence. |
...
| Source-locked transactional soak | Confirm no source drift, semantic exercise, and current-source fingerprint | PASS for deployed candidate `a550ca8`: the 1,800.026-second pair preserved the exact production fingerprint and recorded zero failures. Shorter `1db2d97a` and prior `e942db6f` pairs remain historical only. |
...
Deployed candidate `a550ca8` passed remote `28,949/0` and the source-locked 1,800.026-second pair. The rebuilt tree passes local `40,160/0` across 21 suites and the bounded 8.003-second soak, but requires a fresh committed remote chain.
