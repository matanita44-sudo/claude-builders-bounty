...
> **Validated public candidate:** source commit `a550ca867506f34856cf337fbe28083a9cdbaec5`, remote matrix `28,949/0`, exports, soak, deployment, and public Chromium all pass in run `33594396541`. The live Pages marker later advanced to hardening checkpoint `e7275f5`; run `33668271115` passed Validate, Web, Android debug, unsigned-iOS scaffold, deploy, and 30-minute soak jobs, and unsigned Xcode 26 Simulator/iPhoneOS compiles passed before capture failed. Its public-smoke job then failed during trailer input, so `a550ca8` remains the latest complete public semantic proof rather than the current live bytes. The current local tree passes editor/import and all 21 suites at `41,194/0`, a source-current 8.00-second soak, a fresh Web export plus complete Chromium 140 whole-path/reload capture, and a fresh unsigned Linux iOS scaffold validation/pack boot. The first-hit tutorial/breach state and bottom-lane story toast have current-source pixel/regression proof; focused red-team review reports 0 P0 / 0 P1. The final remote chain remains pending.
...
| Evidence | Result |
|---|---|
| Linux headless Godot main suite | Current local `2,872 passed, 0 failed`; deployed `a550ca8` remote `2,726/0` |
| Current local 21-suite matrix | Main `2,872`; backend `100`; upgrades `120`; tutorial `198`; mechanics `3,541`; compiler `15,515`; defender `354`; live defender `212`; travel `685`; integration `4,131`; organs `7,354`; Titan attack specs `1,045`; meta `111`; audio `686`; story `164`; story presentation `280`; visual `651`; story canon `476`; Story Overlay `34`; Player Presentation `17`; localized layouts `2,648` — all zero failures |
| Current local matrix total | `41,194 passed, 0 failed`; editor/import + 21 suites produced exact sentinels with zero unexpected engine `ERROR`, `SCRIPT ERROR`, or parse-error lines. Inventory: 23 scenes / 21 standalone suites / one nested / one soak. The latest exact tree is not yet committed or remotely rerun. |
| Current local Web export | PASS — release PCK `15,520,528` bytes / SHA-256 `51a74e260575c1011517c0b97d72ab2402bbd9c2696081516c6be1e2d1f670f8`; WebAssembly `39,514,754` bytes. Required files, legal pages, export exclusions, local HTTP runtime smoke, semantic whole-path/reload capture, and both screenshot validators pass. No current deployment or remote-CI claim is made for these bytes. |
| Fresh local Chromium capture | PASS on Chromium `140.0.7339.16` headless-shell, production fingerprint `68c2f81fe51fa09efa0ab25f190de482646fa865b3562ae32f604b656e8bd01b`: 9/9 semantic 540×960 stages and 9/9 unscaled 1320×2868 review stages; exterior → breach → organ selection → internal route → organ destruction → mutation → changed exterior; exact primary-save tutorial/mutation counts after same-context reload; zero page errors, crashes, or request failures. A fresh-profile non-QA capture proves the actual three-line English first-breach story toast is fully visible with no overlap or clipping. Stage/store validators PASS. This is local automated desktop-browser QA, not Mobile Safari/Chrome, native iOS, physical-device, submission-ready screenshot, deployed-public, or current remote-CI evidence. |
| Current local bounded soak | `8.00s`; 99 iterations/projectile cycles; 10 restarts/Dives; six saves; 530 queue events; peak 540; all seven movement models; zero orphan nodes; zero failures. Fingerprint remained `68c2f81f…`. Local Linux-headless structural evidence only. |
| Fresh local unsigned iOS scaffold | PASS for Linux structural scope — 48 files / 405,237,476 bytes; exported PCK `15,520,624` bytes / SHA-256 `28fb6f2c4e66e1ceb13ba86c05604cfbdb0fe900b91fa0bcffe59f201b8e05d2`. Source, 16 exact RGB/no-alpha icon slots, launch-pixel parity, project metadata, `Info.plist`, privacy manifest, one-target control surface, and 120-frame exported-pack boot pass. No Xcode compile/archive, signature, Simulator/device install, or TestFlight result is inferred. |
...
| Save isolation | Guard test without the flag exited 1 with `0 passed, 1 failed`; the prior explicit guard run with `INFINIDIVE_TEST_ISOLATED=1` plus temporary `XDG_DATA_HOME` exited 0 with `2,868/0`, and the current isolated main matrix is `2,872/0`; no production/player profile used |
...
| Paused breach-entry regression | PASS in the current `2,872/0` main suite — while `BREACH_OPEN` is paused, `_request_dive()` is rejected and the pause overlay remains authoritative; manual resume restores the legal Dive path. |
| Tutorial breach-state regression | PASS in the current `2,872/0` main suite — chip damage retains `BREAK ARMOR` while Dive is locked; only `_open_breach()` advances to `ENTER THE GIANT` while Dive is available. Current-source 540×960 QA and non-QA story-toast frames confirm the bottom-center lane does not cover the Titan, Diver, or action rings. |
...
|---|---|---|---|
| Web export artifact | HTML, JavaScript, WASM, PCK presence, excluded tooling/adaptive sources, unresolved-shell token checks, and legal-page paths | PASS (remote checkpoint; current local export/runtime scope) | Actions run `33594396541`, Web job `100135597973`, retains the complete `a550ca8` artifact proof. Hardening run `33668271115` passed Web export and deployed `e7275f5`, although its later public-trailer step failed. The fresh local export PCK is `15,520,528` bytes / `51a74e260575c1011517c0b97d72ab2402bbd9c2696081516c6be1e2d1f670f8`; static/legal checks and the local Chromium 140 whole-path/reload capture pass. No deployment or current remote-CI claim is made for these bytes. |
| Local Chromium 140 current-source capture | Complete semantic gameplay, same-context reload, touch delivery, 540×960 stage evidence, and unscaled 1320×2868 review captures | PASS (local automated browser QA) | Production fingerprint `68c2f81fe51fa09efa0ab25f190de482646fa865b3562ae32f604b656e8bd01b`; 9/9 semantic stages and 9/9 unscaled review stages; stage/store validators PASS; changed exterior reached with Hunter Eye destroyed, `homing_eye` degraded, and `blinded_hunter_eye` rendered; same-context reload restored primary-save tutorial count 8 and mutation discovery count 1. Smoke and the separate non-QA story capture record zero browser errors. This is not Mobile Safari/Chrome, native iOS, physical-device, submission-ready screenshot, deployed-public, or current remote-CI evidence. |
...
| iOS Xcode project | Sanitized unsigned iPhone-targeted scaffold | PARTIAL / BLOCKED | Run `33668271115` passed its unsigned-scaffold validation and Xcode 26 compiled unsigned Release x86_64 Simulator and arm64 iPhoneOS builds. A fresh current-tree Linux scaffold contains 48 files / 405,237,476 bytes, declares exact `CFBundleLocalizations = [en, he]`, passes source/icon/launch/metadata/`Info.plist`/privacy/control-surface validation, and boots its 15,520,624-byte exported PCK for 120 headless frames. Archive, signature, physical-device install, and TestFlight remain blocked. |
...
Deployed candidate `a550ca8` passed remote `28,949/0` and the source-locked 1,800.026-second pair. Remote checkpoint `e7275f5` passed Validate, Web, Android debug, unsigned iOS scaffold, and a newer 30-minute soak; its native compile passed but capture did not. The latest tree passes local `41,194/0` across 21 suites, the bounded 8.00-second soak, a complete Chromium 140 whole-path/reload capture with both screenshot validators, and the fresh unsigned Linux iOS scaffold's declared structural validators/pack boot. Focused current-source red-team review reports 0 P0 / 0 P1; the committed remote chain remains pending.
...
1. Push the locally validated presentation/native-capture fixes and complete Chromium evidence, then require both Xcode 26 Simulator lanes to complete install/launch/terminate plus the full source-bound image sequence.
