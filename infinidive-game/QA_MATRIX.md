...
| Evidence | Result |
|---|---|
| Linux headless Godot main suite | Rebuilt local `2,733 passed, 0 failed`; deployed `a550ca8` remote `2,726/0` |
| Current rebuilt 19-suite matrix | Prior 15-suite set `28,973/0` (`main 2,733/0`, `backend 99/0`, remaining prior-suite counts unchanged); visual regression `315/0`; story canon `476/0`; Story Overlay `34/0`; Player Presentation `17/0` |
| Current rebuilt local matrix total | `29,815 passed, 0 failed`; editor/import + 19 suites produced 20 driver logs and 19 sentinels with zero engine `ERROR`, `SCRIPT ERROR`, or parse-error lines. Inventory: 21 scenes / 19 suites / one nested / zero imported / one soak. The session-local logs were verified but are not retained as repository artifacts; remote proof remains pending. |
| Current local bounded soak | `8.01s`; 99 iterations; 10 restarts/Dives; 99 projectile cycles; 530 queue events; peak 540; zero failures; transaction-recovery self-test PASS. Local structural evidence only. |
...
| Strict harness boundary | Inventory: 21 scenes / 19 standalone suites / one nested relaunch / zero imported / one soak. Editor plus 19 suites produced 20 driver logs and 19 exact sentinels; process failure, engine/script/parse errors, count drift, stale inventory, or invalid soak evidence fails closed. |
...
| Save isolation | Guard test without the flag exited 1 with `0 passed, 1 failed`; the rebuilt suite with `INFINIDIVE_TEST_ISOLATED=1` plus temporary `XDG_DATA_HOME` exited 0 with `2,733/0`; no production/player profile used |
...
| Paused breach-entry regression | PASS in the rebuilt `2,733/0` main suite — while `BREACH_OPEN` is paused, `_request_dive()` is rejected and the pause overlay remains authoritative; manual resume restores the legal Dive path. |
...
| iPhone 16 Pro Max Simulator | Xcode 26 compile, install, launch, survive, terminate, and capture exact 1320×2868 frame | NOT RUN | The rebuilt workflow defines this lane locally, but no remote job or artifact has passed. Do not treat validator self-tests or YAML parsing as native-app evidence. |
...
Deployed candidate `a550ca8` passed remote `28,949/0` and the source-locked 1,800.026-second pair. The rebuilt tree passes local `29,815/0` across 19 suites and the bounded 8.01-second soak, but requires a fresh committed remote chain.
