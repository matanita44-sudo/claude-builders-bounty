...
The reconciled Android debug export was inspected and requests exactly `android.permission.VIBRATE`; no other permission appears in that artifact. The unsigned reconciled-tree iOS project also passed its current structural/privacy-manifest checks. Both must still be re-audited after production signing and whenever export templates or SDKs change, because the final AAB/archive can differ from development artifacts.
