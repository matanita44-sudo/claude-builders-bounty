...
- The analytics abstraction has **no network transport**. Local diagnostics default to off. If the setting is enabled, events are written only to a local queue; disabling it persists opt-out before deleting that queue and retries cleanup at the next boot if removal fails.
...
| Optional local diagnostics queue | Allowlisted gameplay event name, primitive event properties, locally generated session ID, UTC timestamp | Recorded only when the player enables `analytics_opt_in` in Settings; intended for local pre-release diagnosis | `user://analytics_queue.json`, capped at 500 events | **No. There is no uploader, HTTP request, vendor SDK, or backend endpoint.** | The setting defaults to false. Turning it off first persists opt-out, then idempotently deletes the queue; an interrupted/failed deletion is reported and retried at the next boot. Confirmed Reset Progress or platform/site-data clearing also deletes it |
...
The player-facing English and Hebrew setting explicitly describes on-device local diagnostics rather than sharing. Disabling it prevents all new events immediately, attempts synchronous queue deletion, and retries deletion during a later boot while the preference remains off.
