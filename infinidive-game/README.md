...
> **Current evidence boundary:** public artifact `9833689840` (4,752,918-byte ZIP; SHA-256 `b5c2034b1398f852846c476f18c49d758c9fc17836607e06983eeb745a1d7f58`) is bound to `a550ca8`, returns HTTP 200 on a live 540×960 canvas, and traverses exterior combat → breach → organ selection → Dive → internal route → Hunter Eye destruction → mutation → changed exterior. A same-context reload restores the primary save with tutorial count 8 and mutation-discovery count 1; no page error, crash, or request failure was recorded. Soak artifact `9833647628` (SHA-256 `4607d9dfb3e5c0fdb53dd2666a10922fd6b90a7ace18ef6db0236a15ef8ac245`) passed for 1,800.026 seconds at fingerprint `a0e456bc31c947cb1db2330e40b5069a323168b2b0ac1df553143bf9e9eb9a2c`, with 25,109 cycles, 2,511 restarts/Dives, and zero failures or orphan nodes. The rebuilt post-`a550ca8` candidate adds accessibility/privacy hardening, public notices, stricter iOS-icon validation, and an Xcode 26 Simulator lane. Its fresh local matrix passes `29,815/0` across 19 suites, but those changes are unrun remotely. None of this substitutes for Mobile Safari/Chrome, human control-feel, native installation/signing, TestFlight, or physical-device evidence.
...
The rebuilt post-`a550ca8` tree passes **29,815 assertions and 0 failures across 19 local suites**: the prior 15-suite set totals `28,973/0`, and the now-standalone visual regression `315/0`, story-canon acceptance `476/0`, Story Overlay `34/0`, and Player Presentation `17/0` complete the matrix. Editor/import plus 19 strict suite drivers produced 20 driver logs, 19 exact sentinels, and zero engine/script/parse errors; inventory is 21 scenes / 19 suites / one nested probe / zero imported probes / one soak. A bounded 8.01-second soak also passed with 99 iterations, 10 restarts/Dives, 99 projectile cycles, 530 queued events, peak 540, zero failures, and transaction-recovery self-test PASS. Deployed `a550ca8` separately retains remote `28,949/0` proof. The rebuilt delta still requires remote CI, export, native Simulator, deployment, and public-smoke evidence.
