...
> **Current status:** version `0.1.0` is a playable public pre-alpha, not a release candidate. Bright Greek-mythic/AION commit [`a550ca867506f34856cf337fbe28083a9cdbaec5`](https://github.com/matanita44-sudo/claude-builders-bounty/commit/a550ca867506f34856cf337fbe28083a9cdbaec5) completed all seven configured jobs in GitHub Actions run [`33594396541`](https://github.com/matanita44-sudo/claude-builders-bounty/actions/runs/33594396541): strict validation, Web export and full-path/reload Chromium smoke, Android debug export/validation, unsigned iOS Xcode scaffold validation, a source-bound 30-minute soak, Pages deployment, and public-host semantic smoke all passed. The public [game](https://matanita44-sudo.github.io/claude-builders-bounty/infinidive/) serves that exact commit. There is still no live backend transport, production-signed native build, store submission, Xcode-compiled Simulator result, TestFlight install, or physical-device QA evidence.

> **Current evidence boundary:** public artifact `9833689840` (4,752,918-byte ZIP; SHA-256 `b5c2034b1398f852846c476f18c49d758c9fc17836607e06983eeb745a1d7f58`) is bound to `a550ca8`, returns HTTP 200 on a live 540×960 canvas, and traverses exterior combat → breach → organ selection → Dive → internal route → Hunter Eye destruction → mutation → changed exterior. A same-context reload restores the primary save with tutorial count 8 and mutation-discovery count 1; no page error, crash, or request failure was recorded. Soak artifact `9833647628` (SHA-256 `4607d9dfb3e5c0fdb53dd2666a10922fd6b90a7ace18ef6db0236a15ef8ac245`) passed for 1,800.026 seconds at fingerprint `a0e456bc31c947cb1db2330e40b5069a323168b2b0ac1df553143bf9e9eb9a2c`, with 25,109 cycles, 2,511 restarts/Dives, and zero failures or orphan nodes. The rebuilt post-`a550ca8` candidate adds four fully authored Titan attack/collapse sets, deterministic result cards, pre-rendered original audio, hardened exact-once saves, bright trailer capture, accessibility/privacy work, public notices, stricter iOS validation, and Xcode 26 Simulator plus protected signed-release lanes. Its fresh local matrix passes `40,160/0` across 21 suites, but those changes are not yet remote or public evidence. None of this substitutes for Mobile Safari/Chrome, human control feel, native signing, TestFlight, or physical-device evidence.
...
The rebuilt post-`a550ca8` tree passes **40,160 assertions and 0 failures across 21 local suites**. The exact suite counts are main `2,860`, backend `100`, upgrades `120`, tutorial `198`, mechanics `3,541`, compiler `15,515`, defender effects `354`, live defender effects `212`, projectile travel `685`, live integration `4,131`, organs `7,008`, Titan attack specifications `855`, meta `111`, audio `684`, story `164`, story presentation `280`, visual `412`, story canon `476`, Story Overlay `34`, Player Presentation `17`, and localized-layout acceptance `2,403`. The main suite now includes 14 fail-closed activation tests for the QA-only native iOS capture controller. Editor/import plus the strict drivers produced exact sentinels and zero unexpected engine/script/parse errors; inventory is 23 scenes / 21 standalone suites / one nested probe / one soak. A source-current bounded 8.003-second soak passed with 98 iterations, 10 restarts/Dives, 98 projectile cycles, six saves, 530 queued events, peak 540, zero failures, all seven movement models, transaction-recovery PASS, and source fingerprint `3f5aed099357c66985222681d6b825819e595dbd5ef960e3566003283c37c59a`. Deployed `a550ca8` separately retains remote `28,949/0` proof. The rebuilt delta still requires remote CI, export, native Simulator, deployment, and public-smoke evidence.
...
The checked-in `Web` preset writes a normal export to `../build/web/`. Historical pre-pivot packages remain under `../../build/semantic-qa-1db2d97a/`; current public candidate artifacts are retained by Actions run `33594396541`:
