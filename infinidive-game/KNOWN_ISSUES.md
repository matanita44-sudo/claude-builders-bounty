...
- The prior public smoke's semantic false-green gap is resolved in code: the QA bridge is exact-query gated and read-only; the smoke now requires a single `run_generation`, valid numeric state, real 12-pixel movement with no Dash-count change, and exact Dash acceptance/charge consumption. The first CI candidate exposed a real activation-boundary defect: its exported game booted at HTTP 200 with a live canvas and zero console/network errors, but the QA snapshot stayed absent. The current candidate reads `window.location.search` through `JavaScriptBridge.get_interface("window")` and uses the global evaluation context for publication/cleanup. Local import, Web export validation, `28,410/0`, and 8/90-second source-bound soaks pass. This is not yet a closed public/browser issue until the corrected candidate's CI-served and post-deploy Chromium jobs retain passing semantic evidence.
