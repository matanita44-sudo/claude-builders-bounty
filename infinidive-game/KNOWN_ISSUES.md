...
### INF-P1-010 — No public/browser/native-install validation

**Status:** Open

Fresh reconciled-tree Web, Android debug, and unsigned iPhone-targeted Xcode outputs now have recorded sizes/hashes and pass their available structural checks. The Web validation is static; there is still no verified public URL or recorded Safari/Chrome runtime smoke. The APK uses the Android Debug signer and has not been installed. The Xcode export at `../build/ios-iphone-current` verifies exact RGB icons and `TARGETED_DEVICE_FAMILY="1"`, with no Team/placeholder value, but has not been compiled, archived, signed, run in a simulator, or installed. The checked-in preset still needs the owner's Team/signing configuration for release export. Background/resume, interruption, and target-device performance profiles remain absent.

**Required fix:** test and deploy the Web export at `https://matanita44-sudo.github.io/claude-builders-bounty/infinidive/`; install the debug APK on declared Android targets; use the separate AAB preset with complete SDK/build-tools 36 and secure signing; supply the Apple Team/signing configuration, compile and archive on macOS/Xcode, then run simulator/device QA without claiming unperformed coverage.
