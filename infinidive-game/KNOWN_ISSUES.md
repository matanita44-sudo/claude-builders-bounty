...
- The former CI false-green gap is resolved: all 23 test scenes are inventoried. Candidate `a550ca8` passed remote `28,949/0`. The rebuilt tree passes local `40,160/0` across 21 suites, editor/import, exact sentinels, and zero unexpected engine/script/parse errors; inventory is 23 scenes / 21 standalone suites / one nested / one soak. It still needs committed remote evidence.
...
Candidate `a550ca8` retains public/remote evidence. The rebuilt local iOS scaffold repairs scheme-to-target binding and passes metadata/icons/launch/privacy validators. A protected signed archive-only workflow and fail-closed archive/IPA validator now pass local self-tests, but the workflow is not runnable until its reviewed registration copy exists on this repository's default branch. Automated upload is hard-blocked pending protected Xcode 26 `altool` JSON-schema calibration. No Android or iOS artifact has been installed; no release AAB, private native signature, Xcode compilation/archive, Simulator/device execution, TestFlight upload, Mobile Safari/Chrome run, or store result exists. The Mac/Xcode lanes remain pending remote push and repository/Apple setup.
...
**Required fix:** commit and remotely validate the rebuilt hardening tree, including Xcode 26 Simulator compile/install/launch/capture, then repeat the complete flow in declared Mobile Safari/Chrome targets. Supply the Apple Team/signing configuration securely, archive/upload to TestFlight, and run physical-device QA without claiming unperformed coverage.
...
Implementation-aligned privacy/store drafts and original brand sources exist. Existing real-runtime stills and trailers show the superseded dark identity and are historical provenance only and explicitly barred from submission. The bright game and then-current legal pages are public from `a550ca8`; rebuilt notices/privacy hardening and a reproducible bright browser-trailer pipeline remain local. No release artifact is store-approved, and final legal/product review has not occurred. There is still no signed iOS archive, TestFlight/physical-device test, complete accepted-dimension native current-identity screenshot set, approved current App Preview, final-binary notice/privacy audit, submitted age/privacy forms, or completed store listing.
