...
- The former CI false-green gap is resolved: all 23 test scenes are inventoried. Candidate `a550ca8` passed remote `28,949/0`; hardening checkpoint `e7275f5` passed remote Validate plus its Web/Android/iOS-scaffold/30-minute-soak jobs. The current tree passes local `41,194/0` across 21 suites, editor/import, exact sentinels, and zero unexpected engine/script/parse errors; inventory is 23 scenes / 21 standalone suites / one nested / one soak. The current 8.00-second bounded soak also passes 99 iterations/projectile cycles, 10 restarts/Dives, all seven movement models, zero orphan nodes, and zero failures. Its fresh local Chromium 140 browser capture completes the whole path and same-context reload with both screenshot validators passing; the repaired tutorial/breach state and unobscured three-line story toast have current-source pixel evidence and a 0 P0 / 0 P1 red-team verdict. Committed remote evidence remains pending.
...
`SafeAreaHelper` is wired to the Nest and run HUD. It converts native `DisplayServer` safe rectangles on iOS/Android and Web CSS `env(safe-area-inset-*)` values into logical coordinates. The current tree fits gameplay through a dedicated world canvas, fits the HUD independently, extends the background across tall/asymmetric bands, and inverts the actual parent canvas transform for touch input including finger offset. Deterministic coverage includes 1320×2868 coordinate mapping. A local Chromium 140 automated run captured and validated all nine semantic stages at 540×960 plus nine unscaled review stages at 1320×2868. This is useful rendered-browser evidence but is not Mobile Safari/Chrome, a native Simulator, notch/Dynamic Island behavior, small/large physical-phone behavior, or physical touch evidence.
...
Candidate `a550ca8` retains deployed public evidence. The signed workflow's byte-identical default-branch registration is complete through PR #1 / merge `4bbdbb3`; branch/tag protection, the `app-store-production` environment, and Apple credentials remain unconfigured. Hardening run `33668271115` passed unsigned-scaffold validation and Xcode 26 compiled unsigned Release x86_64 Simulator and arm64 iPhoneOS builds, then failed during screenshot-loop transport timing. Debug run `33668271147` compiled/launched and captured the current-bright Nest, then failed at the Titan snapshot when a nullable value reached `String()`. Both capture defects are fixed locally with regression coverage and await rerun. A fresh current-tree Linux scaffold contains 48 files / 405,237,476 bytes; it declares exact `CFBundleLocalizations = [en, he]`, passes source/icons/launch/metadata/`Info.plist`/privacy/control-surface validation, and boots its 15,520,624-byte PCK (SHA-256 `28fb6f2c4e66e1ceb13ba86c05604cfbdb0fe900b91fa0bcffe59f201b8e05d2`) for 120 headless frames. This is unsigned structural evidence, not Apple-platform execution. The fresh Web export and local Chromium 140 whole-path/reload capture pass at production fingerprint `68c2f81fe51fa09efa0ab25f190de482646fa865b3562ae32f604b656e8bd01b`, including 9/9 540×960 semantic stages, 9/9 unscaled 1320×2868 review stages, exact touch delivery, and zero page errors/crashes/request failures. This remains local automated desktop-browser QA. Automated upload remains hard-blocked pending protected Xcode 26 `altool` JSON-schema calibration. No release AAB, signed iOS archive, physical-device install, TestFlight upload, Mobile Safari/Chrome run, deployment of the current local tree, current remote-CI proof, or store result exists.
...
**Required fix:** commit and remotely validate the local fixes and Chromium evidence, complete the Xcode 26 Simulator install/launch/capture/terminate artifact, and repeat the browser flow in declared Mobile Safari/Chrome targets. Configure branch/tag protection and the protected release environment, supply the Apple Team/signing configuration securely, archive/upload to TestFlight, and run physical-device QA without claiming unperformed coverage.
...
Implementation-aligned privacy/store drafts and original brand sources exist. The bright Greek-mythic app icon and Olympus launch screen have been rebuilt, their iOS derivatives pass exact size/RGB/no-alpha and launch-pixel checks in a fresh local scaffold, and the large source-only icon asset is excluded from runtime packages. Existing real-runtime stills and trailers still show the superseded dark identity; they are historical provenance only and explicitly barred from submission. A fresh local Chromium 140 run produced nine current-source unscaled 1320×2868 review images and both screenshot validators pass, but the validator explicitly classifies them as non-submission-ready and non-target-device evidence. The bright game and legal-page set, including rebuilt Notices and Draft Terms, are deployed from `e7275f5`; the latest complete public gameplay proof remains `a550ca8` because the later public job failed during trailer input. No release artifact is store-approved, and final legal/product review has not occurred. There is still no signed iOS archive, TestFlight/physical-device test, accepted native current-identity screenshot set, approved current App Preview, final-binary notice/privacy audit, submitted age/privacy forms, or completed store listing.
