...
**Required fix:** extend both passing CI-served and deployed-public movement/Dash semantic smokes through breach, Dive, organ destruction, outside return, boss-system change, and reload persistence; repeat that complete flow in declared mobile Safari/Chrome targets. Install the debug APK on declared Android targets; use the separate AAB preset with complete SDK/build-tools 36 and secure signing; sanitize unused iOS privacy keys, supply the Apple Team/signing configuration, re-export/compile/archive on macOS/Xcode, then run simulator/device QA without claiming unperformed coverage.
