...
- The prior public smoke's semantic false-green gap is resolved in code: the QA bridge is exact-query gated and read-only; the smoke requires exactly one new `run_generation`, valid numeric state, real movement with no Dash-count change, and exact Dash acceptance/charge consumption. The first CI candidate exposed a real activation-boundary defect: its exported game booted at HTTP 200 with a live canvas and zero console/network errors, but the QA snapshot stayed absent. The corrected candidate reads `window.location.search` through `JavaScriptBridge.get_interface("window")` and uses the global evaluation context for publication/cleanup. Run `33572931398` now closes the **CI-served and deployed-public movement/Dash semantic portions only**. CI-served artifact `9825704303` records exact `run_generation 0 -> 1`, `267.402` pixels of movement, Dash `0 -> 1`, and zero recorded failures. Deployment job `100078099551` published marker `73a3f4aad29a2d3900fe55e94ba4cfde6885d42a`; public-smoke job `100078147875` and artifact `9826433759` then recorded HTTP 200, a live 540×960 canvas, `269.180` pixels of movement, Dash `0 -> 1`, charge `1 -> 0`, `3/3/3` touch events, and zero page, crash, network, or critical failures. Breach/Dive/organ-return semantics, reload persistence, mobile Safari/Chrome, native installation, physical-device behavior, and human control feel remain open and are not implied by these results.
...
- The AION story layer is integrated in the current working tree rather than remaining data-only: the first eligible CRONUS Story launch presents the skippable English-first prologue, first movement awakens the hand-cast AION SPARK on the visibly unarmed Keeper, fresh Story chapters present their intro, and an explicit first-Story-clear result gates the shard-restoration beat. Daily, Friend, and Abyss wins cannot suppress or fabricate those Story beats. Story Service `164/0`, Story Presentation `280/0`, story-canon `476/0/0`, isolated Story Overlay `34/0`, and Player Presentation `17/0` pass locally. The authored `first_breach` beat is still data-only, and public/browser/device presentation plus human pacing/comprehension remain unverified.
...
- The former CI false-green gap is resolved: all scenes are inventoried and isolated; process failure, engine/script/parse errors, sentinel/count drift, stale inventory, or invalid/missing soak evidence fails closed. The current working-tree 15-suite matrix passes 28,949/0 locally. New committed fingerprints and remote evidence are pending; older `1db2d97a` / `ff2530d3` hashes remain historical candidate evidence only.
...
- Current-candidate run `33572931398` completed successfully, including source-bound long-soak job `100071482078`. Artifact `9826413723` (9,008-byte ZIP; `1290cf7b8cf81b64dd6f0b43e55739f4f3bbaa24d4b40a083d6dbf1296b03a63`) records 1,800.035 seconds, unchanged production fingerprint `1db2d97aa0852a415ee4a76e3d4be6ea20949140dfcdee46e49d295e55525e8e`, 27,043 iterations/cycles, 2,705 restarts/Dives, 1,353 save writes / 22 reloads, 3,224 queue events / 24 queue reloads / final 500, peak 540, exact 7/7 model coverage, complete transaction `9f396a0becf39225c7580401`, and zero failures. The retained JSON SHA-256 is `be3c3d795d31e9581e6fa108f462f58cd3264d9f24aff4d338a79123e485ed09`; Markdown/bound SHA-256 is `d24fe59b37296f6006bb27dc18b61f3ff4996bbfdfc354d852af1377c3ab64b3`. This is Linux headless structural evidence, not target-device performance or human-play evidence.
...
The event-driven tutorial tracks ten steps and sets `tutorial_complete` only after all ten are understood. Replay can be requested for the next run; its presentation state persists through the Run-to-Nest boundary and the Forge purchase completes the tenth replay step. The defense step now accepts either a live Dash or a real telegraphed volley that ends without a hit or Dash. The integrated AION prologue, unarmed first movement, Spark awakening, chapter intro, and first-clear victory beat have automated presentation/decision coverage, but their pacing and comprehension have not been tested with people. The authored `first_breach` story beat is not yet presented during gameplay. The tutorial remains observational rather than gated.

**Required fix:** add targeted coachmarks/gates where observation alone is insufficient, connect or deliberately remove the authored `first_breach` story beat, then validate opening-story pacing, Spark comprehension, control comprehension, and time-to-first-Dive with real testers.
...
Current candidate packages exist outside the repository at `../../build/semantic-qa-1db2d97a/`: 11,022,194-byte Web ZIP `a8504d0c0630dced1c3892c971a0d3f9be67844250927b8bf9a80340ebc98e4f`, 29,063,530-byte debug APK `e3418d97a535cf4e36bb7c1cf05e4f7db4e722a774409e29df6f80b33786cf03`, and 98,500,694-byte unsigned iOS scaffold ZIP `54bb7d9f866608773470c90f8b8d953b4668f2bc3914086e2e96e131b0f85b9d`; its embedded current-source PCK is 629,332 bytes with SHA-256 `8133fcd7ebbb071b20f30ca65a43cb35488b6fff2335915ef47d710693c33a88`. `BUILD_EVIDENCE.md`, archive integrity, static Web checks, local HTTP checks, APK structure/signature validation, PCK parity, and `SHA256SUMS` pass. The APK is debug-signed and was not installed. The historical iOS ZIP is only an unsigned scaffold plus the current PCK; it was not compiled, archived, signed, installed, or submitted. It still contains empty Camera, Microphone, and Photo Library descriptions plus the stock Godot splash, so it remains unsuitable for archive/store claims. The current working tree instead has a fail-closed post-export plist sanitizer, exact privacy-manifest validator, custom 2x/3x launch-pixel validator, and an unsigned Linux scaffold CI path that injects a fake Team ID only into a temporary copy, scrubs it before retention, and refuses an artifact containing the placeholder. A fresh local working-tree export passed those checks, including exact required-reason APIs and stock-splash rejection; the new remote job/artifact still needs a passing committed run, and final macOS archive inspection remains mandatory. Current candidate commit `73a3f4aad29a2d3900fe55e94ba4cfde6885d42a` passed run `33572931398`, including validation, Web export, Android debug export, deployment job `100078099551`, and public semantic-smoke job `100078147875`. CI-served artifact `9825704303` records exact `run_generation 0 -> 1`, `267.402` pixels of movement, Dash `0 -> 1`, and zero recorded failures. Public artifact `9826433759` (92,257 bytes; `014fe807c93ed3d03d7c6cfebac201faec2986324f8a576be2fbb6967ae9c020`) records deployment marker `73a3f4aad29a2d3900fe55e94ba4cfde6885d42a`, HTTP 200, a live 540×960 canvas, `269.180` pixels of movement, Dash `0 -> 1`, charge `1 -> 0`, `3/3/3` touch events, zero page/crash/network/critical failures, and distinct frame hashes `0da738405c7983f4d795655ada7d31acbbc4d973724b404b5651034300ba3eb9` / `7ca6ba2e9fc87aa5ac7ea3629138d23d3cd9a7bb24b3abf39c7c839971a09633`. These narrow results do not establish the breach/Dive/organ-return flow, reload persistence, a mobile browser, a native installation, physical-device performance, production signing, or any store result.

Historical evidence remains separate: runtime-evidence commit `380b6d4b632e9d507ea42075714d0f18d6cdb74f` passed run `33565500042`, including deployment and a narrow synthetic public-host touch/frame smoke for the earlier `e942db6f` candidate. Its public game/privacy/support/PCK/WASM requests returned HTTP 200. It remains prior-source evidence and is not substituted for the current `1db2d97a` candidate evidence above.

**Required fix:** extend both passing CI-served and deployed-public movement/Dash semantic smokes through breach, Dive, organ destruction, outside return, boss-system change, and reload persistence; repeat that complete flow in declared mobile Safari/Chrome targets. Install the debug APK on declared Android targets; use the separate AAB preset with complete SDK/build-tools 36 and secure signing; supply the Apple Team/signing configuration, regenerate the Xcode project, verify the sanitized final `Info.plist`, compile/archive on macOS/Xcode, then run simulator/device QA without claiming unperformed coverage.
...
The main headless suite completes all four bosses through all six organ orders, destroys each final core, verifies rewards, and prevents duplicate banking. A checked-in schema-1 fixture migrates to schema 6. Simulated application pause and close persist a banked result synchronously, a fresh process rejects replay with exact totals, an injected save failure blocks navigation until exact-once reward banking succeeds, and five consecutive Abyss continuations preserve build/repair/depth/scaling without Story-shard leakage. Remaining gaps are an installed update from a prior TestFlight/App Store build and native interruption timing around combat/reward writes.
