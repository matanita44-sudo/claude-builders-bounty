...
- The former CI false-green gap is resolved in the current workflow: all scenes are inventoried and isolated; process failure, engine/script/parse errors, sentinel/count drift, stale inventory, or invalid/missing soak pair fails closed. The final editor-plus-13-suite pass produced zero error lines and 28,410/0 assertions. Production/tests-CI fingerprints are `8e9810de2615332713f86f47bf2f28f38728fb75e51ea5bfaf2d16760927d863` / `db398ae7804cf75e6741e13380993f9425d42b3e44de8da62631f159595f1597`. The soak validator recomputes current source and rejects stale, incomplete, or cleanup-pending `PASS` evidence.
...
Verified packages now exist under `build/final-0.1.0-8e9810de`: Web ZIP `4b0ab051…`, debug APK `d089cebf…`, and unsigned iOS ZIP `5e3276c7…`; `BUILD_EVIDENCE.md` and `SHA256SUMS` pass. Web static/local-HTTP validation, Android structural validation, and the iOS PCK headless main-pack probe pass. No current browser canvas, native install, production signing, physical-device, or store result is claimed. Authenticated Pages Source is `GitHub Actions`, but public paths remain recorded as HTTP 404 and no deployment is proven.
