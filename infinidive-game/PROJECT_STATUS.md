...
> **Current truth:** deployed commit `a550ca8` is the current public evidence candidate. All seven jobs in run `33594396541` passed. The rebuilt delta now passes a fresh local 19-suite matrix at `29,815/0`, editor/import, 20 driver logs, 19 sentinels, and zero engine/script/parse errors; it remains uncommitted/unrun remotely and is not public evidence. Mobile browsers, signed native installs, TestFlight, and physical-device execution remain open.
...
| Rebuilt post-`a550ca8` candidate | Accessibility/privacy hardening, public notices, stricter iOS icon checks, and an Xcode 26 Simulator lane | Local editor/import + 19-suite `29,815/0` PASS; 20 driver logs, 19 sentinels; inventory 21 scenes / 19 suites / one nested / zero imported / one soak; zero engine/script/parse errors. Remote CI, Simulator artifact, and deployment remain pending. |
| Current local bounded soak | 8.01 seconds; 99 iterations; 10 restarts/Dives; 99 projectile cycles; 530 queue events; peak 540; zero failures | Transaction-recovery self-test PASS. Local structural evidence only. |
...
| Strict local CI harness | Inventory valid: 21 scenes / 19 standalone suites / one nested relaunch / zero imported / one soak | Editor plus 19 suites produced 20 driver logs and 19 exact sentinels; zero engine/script/parse errors. Missing/stale scenes and count drift fail closed. |
...
- The rebuilt tree records 29,815 assertions across 19 local suites with zero failures. The prior 15-suite set is `28,973/0`; visual `315/0`, story canon `476/0`, Story Overlay `34/0`, and Player Presentation `17/0` are standalone. Editor/import, 20 driver logs, 19 sentinels, and strict zero-error scanning pass. Deployed `a550ca8` separately retains remote `28,949/0` proof.
...
| iOS testing build | Candidate `a550ca8` passed remote unsigned-scaffold job `100135598028`, including strict plist/privacy/launch/Bundle ID/version/empty-entitlement checks and fake-Team-ID scrubbing. A reconstructed Xcode 26 Simulator lane exists only as local intended work and has not run remotely. Archive/sign/TestFlight still require the owner's Team ID, certificate, provisioning, and Apple-account access. No Xcode-compiled app, Simulator/device install, archive, sign, or upload is current evidence. |
