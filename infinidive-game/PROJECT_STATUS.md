...
| `../../build/web/` | current-working-tree export: HTML 2,618 bytes / SHA-256 `190b5852ff3c94b4d2e6ce2849bdbc92cb64e77c6cb972d35165b9cf544713b5`; PCK 440,384 / `f9acf67d893ab6513f2b7fde450fcd2129331ba8252b3464132d94892b85b9d4`; WASM 39,514,754 / `fc74679e3b97f76878947fcd4fbe1268cbfa6188182a2e33bbc3f5dc9bfa57d0` | Real local Godot Web evidence export passed static validation; build tooling and adaptive sources are excluded. It is separate from the checked-in preset's default `../build/web/` target and the older remote artifact below, and does not itself prove browser runtime or public deployment. |
...
| `../.github/workflows/infinidive-ci.yml` | remote run 33498494206: `validate`, `web-export`, and `android-debug` succeeded for commit `374bdb5`; Pages rerun job 99862926627 failed again at Create Pages | Remote build/test/export evidence exists only for the named older commit. Deployment did not start because `Create Pages site` again returned `Resource not accessible by integration`; direct checks of the game, support, and privacy URLs return HTTP 404 and no canvas. |
...
- A current-working-tree local Godot Web export passes static validation. The older remote commit-bound export passed CI-served Playwright/Chromium boot smoke; current-tree exported-canvas touch/gameplay, mobile browsers, and the public URL remain untested.
...
| Public playable URL | `https://matanita44-sudo.github.io/claude-builders-bounty/infinidive/` is the configured isolated branch fallback. Actions run 33498494206 passed the Web build and CI-served browser boot smoke for commit `374bdb5`, but deploy rerun job 99862926627 again failed because `Create Pages site` returned `Resource not accessible by integration`. Direct verification returns HTTP 404 for the game, support, and privacy URLs and finds no canvas. |
...
1. Resolve GitHub's Pages-site creation/Actions permission for this repository, rerun deploy, then verify HTTP 200 plus a live canvas at `https://matanita44-sudo.github.io/claude-builders-bounty/infinidive/`; rerun job 99862926627 still returned `Resource not accessible by integration`, and the game/support/privacy URLs remain HTTP 404.
