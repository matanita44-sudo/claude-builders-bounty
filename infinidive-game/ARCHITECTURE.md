...
Web semantic automation uses a deliberately narrow boundary in `Main` and `RunScene`. `Main` checks the exact `infinidive_qa=1` query only on Web, publishes a JSON-safe whitelist at 10 Hz, assigns a monotonic revision and ephemeral run generation, and deletes the global snapshot on exit. `RunScene.qa_snapshot()` reports only view/state validity, position, control/movement observation, Dash state, and elapsed time; non-finite or impossible state sets `numeric_state_valid=false` and serializes non-finite numbers as `null` rather than turning corruption into a plausible zero. No JavaScript-to-Godot callback exists, and run IDs, seeds, saves, profile/currency state, challenge codes, account identifiers, and analytics data are not published. The Chrome smoke consumes this contract to distinguish canvas event delivery from semantic movement and Dash acceptance.

The current frozen local matrix is 28,410/0 across 13 suites: main 2,631; backend/offline 82; permanent upgrades 120; tutorial 198; room mechanics 3,541; pure compiler 15,515; pure defender effects 354; live defender effects 212; projectile travel 685; live integration 4,131; organ transformations 325; meta goals 111; and adaptive audio 505. The room-runtime subset is 24,438/0 across its six invocations. Editor import and every suite passed the strict wrapper with zero engine error lines. The canonical production and tracked tests/CI inventories are fingerprinted as `7fb2ddb25e31c6711e75c7c96fd9f7d6be00863c46b327c04c51a8698e7b9363` and `d30ece3bad7997b749dedce70ff636575425d92aac017f6c9204ac3d6e99bc58`. The production calculation deliberately excludes `assets/store/gameplay/raw/`: its continuous provenance capture is local-only, untracked, and non-exported, while every tracked production and exported game input remains in scope. Travel/live coverage includes all seven accepted travel IDs at 30/60 Hz, player homing at 30/60/120 Hz, complete frozen previews, preview-digest rejection, actual-homing suppression, lane-topology rejection, swept collision before retirement in first-contact order, nonlinear/homing subsegments without safe metadata, first-arena-exit terminal behavior under exact extreme hitch cases, gravity-vector preservation, full-live-radius safe-disk clearance, compiler-signed owner/cycle isolation, bounded fallback, cleanup, and pool reuse.
...
The current 8.239-second and 90.184-second structural-soak pairs use unchanged start/end fingerprint `7fb2ddb25e31c6711e75c7c96fd9f7d6be00863c46b327c04c51a8698e7b9363`, execute 7/7 requested-versus-executed travel models, complete their two-phase transactions, and report zero failures. The bounded pair records 91 cycles, 10 restarts / 10 Dives, 6 saves, and transaction `b04902b7a88fbfcb2eda26fe`; its JSON/Markdown hashes are `ed082e62f4ac9ed56c5854c2e850a981227d8210807c3ece2bdad8324b623c5a` / `b653617f726f1d4ad51e45214bc10b6225bd4a5afa64bf0002e632a0018daa22`. The 90-second pair records 1,141 cycles, 115 restarts / 115 Dives, 58 saves, 634 offline events / 3 queue reloads / final 500, stable delta 96,380 bytes, slope 223,392.732278109 B/min, and transaction `1051aee6fe155c7309536e30`; its hashes are `900352107276b822a135fa13aa146f9ec6837730affa0feee937dab389ce34f9` / `f0e4448d219ac2618c27e3369f0fb08d67d05555639a66989d5deb5fe41e2005`.

The previous-source 30-minute pair was completed by Actions run `33559947112`, long-soak job `100030992601`, and retained in artifact `9822001845`. It remains valid only for fingerprint `e942db6f8a0f1a47518e8afb468f77c651001a2028fcad0820e71c4d993a6382`; the `7fb2ddb2` replacement is mandatory and pending. That historical pair records 26,676 cycles, 2,668 restarts / 2,668 Dives, 1,335 saves / 22 reloads, 3,188 offline events / 24 queue reloads / final 500, 1,636,943 player and 3,216,180 enemy projectile spawns, peak 540, baseline/final nodes 11/10, orphan peak 0, stable delta 3,667,676 bytes, and slope 84,517.9971965645 B/min. This remains Linux headless structural evidence: it does not prove human control feel, browser/device presentation, target-device performance, background/force-close behavior, migration from a previously shipped fixture, repeated Abyss depths, or native installation.

`export_presets.cfg` defines Web, Android debug APK, a separate Gradle Android AAB, and iOS outputs. Both Android presets request only the normal `android.permission.VIBRATE` permission required by optional haptics. Canonical packages under `../build/final-0.1.0-e942db6f/` are named in `PROJECT_STATUS.md`; `BUILD_EVIDENCE.md` and `SHA256SUMS` pass, as do Web static/local-HTTP validation, the current CI-served and public-host headless-Chrome boots, public-host synthetic delivery of canvas-targeted touch events with a byte-verified rendered change, Android structural validation, and the iOS PCK headless main-pack probe. No complete semantic touch-gameplay browser run, Mobile Safari/Chrome result, AAB, native install, production signature, Xcode compile/archive, simulator run, physical-device result, or store upload is claimed.

The workflow at `../.github/workflows/infinidive-ci.yml` imports, tests, exports Web and Android debug, browser-smoke-tests the Web export, deploys Pages from `infinidive-production`, runs a commit-bound post-deploy public-host smoke, and can run an explicitly requested 30-minute source-bound soak. Its current test harness discovers every Godot test scene, validates an explicit manifest of 13 standalone suites plus the nested relaunch probe and soak scene, isolates each runnable scene's data root, authenticates an exact sentinel/assertion count, and rejects every engine `ERROR`, script error, or parse error even after an apparent passing summary. Soak reports are a two-phase JSON/Markdown transaction: the writer validates the complete schema and semantic `PASS` coverage, binds the exact Markdown SHA-256 into JSON, sets `report_transaction_complete` only after cleanup, restores the previous pair symmetrically on either-side failure, supports positive fractional durations, and persists partial/early or source-change diagnostics as valid `FAIL` pairs. Open/write/verify/first-commit/second-commit/cleanup, truncated-pair, mixed-backup, fractional-duration, and source-change paths are exercised. The CI validator recomputes the current production fingerprint, rejects stale `PASS` and recovery evidence, self-tests `PASS`, diagnostic, and strict negative fixtures, and requires the exact result/transaction/completion/bound hash plus requested duration and semantic exercise counts. Runtime commit `380b6d4b632e9d507ea42075714d0f18d6cdb74f` is pushed remotely. Actions run `33565500042` passed validation, current Web export, CI-served headless-Chrome smoke, Android debug validation, Pages deploy job `100049011404`, and public-host smoke job `100049076641`; the latter recorded HTTP 200, a live 540×960 Godot/WebGL2 canvas, zero page errors, canvas touch counts 3/3/3, and a byte-verified rendered change in artifact `9823113363`. Its optional long-soak job was skipped by design after the retained current-source 30-minute pair had passed validation; that pair remains bound to run `33559947112`, job `100030992601`, artifact `9822001845`. This is synthetic canvas-delivery evidence, not complete semantic touch-gameplay, Safari/mobile-browser, native-install, or physical-device coverage.
