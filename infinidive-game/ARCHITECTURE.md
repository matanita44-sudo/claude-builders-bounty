...
The committed workflow at `../.github/workflows/infinidive-ci.yml` imports, tests, exports Web and Android debug, browser-smoke-tests the Web export, and attempts Pages deployment on `infinidive-production`. Actions run 33498494206 passed validation, Web export/browser boot, and Android debug export only for commit `374bdb5cb8de7f4622917a343e379ff4cfd26232`; it does not validate the current working tree. Pages rerun job 99862926627 again failed because the integration could not create the Pages site. The game, support, and privacy URLs return HTTP 404 and no canvas, so no public-host success is recorded.
