...
|---|---:|---|
| `bosses.json` | 4 | Stats, organs, abilities, colors, rewards, procedural silhouettes, four authoritative intact tuning scalars per ability, and 12 loss contracts: seven degraded replacements plus five shutdowns |
...
- `BossPatternPlanner` deterministically selects an attack and declares its executor. ACTIVE organ attacks carry only strict `intact_tuning` (`projectile_budget`, `telegraph_seconds`, `base_speed`, and `damage`) into `TitanAttackSpecFactory`; they do not retain a generic planner pattern. The Factory keeps the 12 ability-specific geometries/effects and consumes those four catalog scalars. BASIC and DEGRADED plans remain bounded `aimed_fan`, `ring`, or `lane` patterns executed only by the planner.
- An ACTIVE attack with missing, extra, malformed, or non-finite tuning, or without a valid Factory plan at spawn time, fails closed instead of falling back to a legacy generic alias.
...
`OrganAbilityMap` maps each organ to one exterior ability, one strict intact tuning record, and one required data-driven loss contract. Destruction is idempotent and always removes the intact ability and its Factory tuning. Seven loss contracts remain runtime-selectable as safer `aimed_fan`, `ring`, or `lane` replacement patterns with bounded projectile counts, speeds, damage, readable gaps, and telegraphs that cannot be shorter than the intact warning. Five loss contracts seal the system and remove it from the attack pool entirely. The legacy `enabled` field continues to mean “intact ability,” while `runtime_enabled`, `status`, `variant`, `strength`, `telegraph_multiplier`, and `pattern` describe the actual post-loss state.

Every launch organ owns a distinct mechanical variant and `visual_token`. `RunScene` consumes the transformed attack contract and preserves cause/wave identity for damage attribution; `BossVisual` draws all 12 specific exterior states; the HUD selects localized English/Hebrew transformed-versus-disabled feedback. The dedicated 7,354-assertion organ suite validates the full catalog, strict intact-tuning/executor split, seven/five loss split, exact described replacement patterns, validation guardrails, live `RunScene` consumption, readable ring telegraph alignment, unique visuals, isolation, and idempotency. The 1,045-assertion Factory suite separately proves all four tuning scalars affect the unique compiler output deterministically and that invalid ACTIVE ownership fails closed. This is logic/render-path evidence, not human readability, fairness, or target-device visual QA.
...
- **Friend Rift:** encodes boss, seed, weapon, difficulty, modifiers, targets, and a short checksum into an `ID1` code. Result sharing is exposed only after Daily/Friend runs whose competitive rules match; Story permanent-upgrade/Assist state and Abyss continuation state fail closed because ID1 cannot reproduce them.
...
The current local matrix is 40,709/0 across 21 suites. Exact changed counts are main 2,868, organs 7,354, Titan attack specs 1,045, and localized layouts 2,408. It covers focused Titan attack specifications, expanded organ transformations, pre-rendered audio, visual, story, localized-layout acceptance, and fail-closed native-capture regressions. Editor/import plus all suite drivers produced exact sentinels and zero unexpected engine/script/parse errors; inventory is 23 scenes / 21 standalone suites / one nested probe / one soak. The current 8.05-second bounded soak passed 97 iterations, 10 restarts/Dives, 97 projectile cycles, six saves, 530 queued events, peak 540, all seven movement models, and zero failures. `a550ca8` separately retains the latest complete public remote 28,949/0 proof and production fingerprint `a0e456bc31c947cb1db2330e40b5069a323168b2b0ac1df553143bf9e9eb9a2c`; the live marker later advanced to hardening checkpoint `e7275f5`, which has newer remote validation/build/soak evidence but an incomplete public-smoke result. The latest local fixes require a fresh remote fingerprint.
...
The latest complete public semantic artifact `9833689840` is 4,752,918 bytes / `b5c2034b1398f852846c476f18c49d758c9fc17836607e06983eeb745a1d7f58`; its evidence marker binds that run to commit `a550ca8`. The report records HTTP 200, a live 540×960 canvas, exterior combat, breach, Hunter Eye selection, Dive, internal rooms, organ destruction, mutation, `homing_eye` degradation, `blinded_hunter_eye` exterior rendering, and same-context reload restoring the primary save with tutorial count 8 and mutation-discovery count 1. It recorded zero page errors, crashes, and request failures. The live Pages marker later advanced to `e7275f5`, whose public job failed during subsequent trailer input. This proves a bounded automated desktop-Chromium path for `a550ca8`, not the current live bytes, Mobile Safari/Chrome, native installation, target-device performance, or human control feel.
...
The retained `a550ca8` 30-minute pair was produced by Actions run `33594396541`, long-soak job `100135598119`, and artifact `9833647628` (9,281 bytes / `4607d9dfb3e5c0fdb53dd2666a10922fd6b90a7ace18ef6db0236a15ef8ac245`). It passed for 1,800.026 seconds with unchanged start/end fingerprint `a0e456bc31c947cb1db2330e40b5069a323168b2b0ac1df553143bf9e9eb9a2c`, 25,109 cycles, 2,511 restarts/Dives, zero failures, and zero orphan nodes. Hardening run `33668271115` contains the newer `e7275f5` source-bound soak evidence.
...
The iOS CI hardening path installs the hash-verified `ios.zip` template, copies the project to an ephemeral directory, and inserts an unmistakable non-secret Team placeholder only into that copy so Godot on Linux can regenerate an unsigned Xcode scaffold. It then atomically sanitizes the main `Info.plist`, injects the exact reviewed `CFBundleLocalizations` array (`en`, `he`), rejects any non-empty Camera/Microphone/Photo description, removes `CFBundleSignature`, requires the exact reviewed required-reason API manifest, and compares decoded generated 2x/3x launch pixels to their RGB source assets. Before bounded artifact upload it blanks the placeholder in `project.pbxproj` and `export_options.plist`, scans every retained file for leakage, and writes an explicit unsigned-Linux evidence note. Simulator log evidence is scoped from the pre-launch timestamp, bound to the exact PIDs returned by every `simctl launch`, and rejects all unreviewed Error/Fault or crash records; only exact per-process/aggregate-bounded Godot and Simulator noise can pass after a bounded log flush. Run `33668271115` proved unsigned Xcode 26 compilation for x86_64 Simulator and arm64 iPhoneOS before its screenshot loop failed; archive, protected signing, physical-device installation, and TestFlight remain separate gates.

The workflow imports, tests, exports, deploys, and smoke-tests the project. Its current inventory classifies 23 Godot scenes as 21 standalone suites, one nested relaunch probe, zero imported probes, and one soak scene. The strict local run produced 22 driver logs and 21 suite sentinels with zero unexpected engine/script/parse errors. Candidate run `33594396541` remains the latest complete deployed/public proof for `a550ca8`. Hardening run `33668271115` advanced the live marker to `e7275f5` and passed Validate, Web, Android debug, unsigned iOS scaffold, deploy, and 30-minute soak; its Xcode compile steps passed but screenshot transport failed, and its public job later failed during trailer input. Debug run `33668271147` reached and captured Nest before a nullable Titan snapshot failed. The local fixes require final remote rerun.
