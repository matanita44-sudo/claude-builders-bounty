...
The current matrix is 28,949/0 across 15 suites: main 2,726; backend/offline 82; permanent upgrades 120; tutorial 198; room mechanics 3,541; pure compiler 15,515; pure defender effects 354; live defender effects 212; projectile travel 685; live integration 4,131; organ transformations 325; meta goals 111; adaptive audio 505; story 164; and story presentation 280. The room-runtime subset is 24,438/0 across six invocations. Editor import and every suite passed the strict wrapper with zero engine error lines. Bright deployed candidate `e9e7a50` is fingerprinted at production/source value `6b7a5fa2f9639c2855874e9f91d3632ab49af274d2cb6b6cbd4a4c2d1c1456a1`. The first-breach/schema-7/Terms/stage-capture follow-up repeats the same suite counts locally but requires a fresh remote fingerprint. Production fingerprinting excludes local-only, untracked, non-exported capture evidence. Travel/live coverage includes all seven accepted travel IDs at 30/60 Hz, player homing at 30/60/120 Hz, complete frozen previews, preview-digest rejection, actual-homing suppression, lane-topology rejection, swept collision before retirement in first-contact order, nonlinear/homing subsegments without safe metadata, first-arena-exit terminal behavior under exact extreme hitch cases, gravity-vector preservation, full-live-radius safe-disk clearance, compiler-signed owner/cycle isolation, bounded fallback, cleanup, and pool reuse.

Candidate source commit `e9e7a50f24914fab8afd1cfe36b2e236c5402e7f` (tree `bbe437d9da3aa8f471421ab5af8761676cbaabe8`) is the source under GitHub Actions run `33590118787`. Validate, Web export/CI-served semantic Chrome, Android debug validation, unsigned iOS scaffold validation, source-bound 30-minute soak, Pages deploy, and public-host semantic smoke all passed. CI-served artifact `9831547931` is 780,235 bytes at the Actions API boundary and its downloaded ZIP hashes to `e26b88701f0da6dba70a0887cc5876b0a0751585bc7a37cc6f53b4fd635a90e7`.

The deployed public artifact `9832202834` is 780,343 bytes / `28ec3a55c1d0ed8c57f6a0587371792d3680048b29d5c4ea69593505c7d79dd7`; its deployment marker binds the public URL to commit `e9e7a50`. The semantic report records HTTP 200, a live 540×960 canvas, exterior combat, breach, Hunter Eye selection, Dive, internal rooms, organ destruction, mutation, `homing_eye` degradation, `blinded_hunter_eye` exterior rendering, and same-context reload restoring the primary save with tutorial count 8 and mutation-discovery count 1. It recorded zero page errors, crashes, and request failures. This proves a bounded automated desktop-Chromium path, not Mobile Safari/Chrome, native installation, target-device performance, or human control feel. It also predates the current schema-7 first-breach/Terms/stage-capture follow-up.
...
The current deployed-candidate 30-minute pair was produced by Actions run `33590118787`, long-soak job `100122969264`, and artifact `9832161116` (9,282 bytes / `cd27506d42293e04d1f8dd21b7caf72428164a013f23df3ec661115b69c0160d`). It passed for 1,800.002 seconds with unchanged start/end fingerprint `6b7a5fa2f9639c2855874e9f91d3632ab49af274d2cb6b6cbd4a4c2d1c1456a1`, exact 7/7 travel models, 25,369 cycles, 2,537 restarts/Dives, 1,270 saves / 21 reloads, 3,057 offline events / 23 queue reloads / final 500, 1,556,637 player and 3,058,437 enemy projectile spawns, peak 540, peak 1,641 objects / 42 nodes / zero orphan nodes, stable delta 3,700,120 bytes, and zero failures. Transaction `65a4f2109909677648587490` is complete; JSON hashes to `86cd0a11be8a2d8e34b52e8f05c7b4ac88223047984fe37344ae506cad870319`, and Markdown/bound hash is `c8f458160826f3c8bdb630be3a0aee33042e07439ae21cd5e84319ee0282dd2c`.
...
The workflow at `../.github/workflows/infinidive-ci.yml` imports, tests, exports Web and Android debug, creates and validates an unsigned iOS scaffold, browser-smoke-tests the Web export, deploys Pages from `infinidive-production`, runs a commit-bound post-deploy public-host smoke, and can run an explicitly requested 30-minute source-bound soak. Its harness inventories all 21 Godot test scenes, validates 15 standalone suites plus imported probes/relaunch/soak roles, isolates runnable data roots, authenticates exact sentinel/assertion counts, and rejects every engine `ERROR`, script error, or parse error even after an apparent passing summary. Soak reports use a two-phase JSON/Markdown transaction with source locking, exact bound hashes, recovery, failure injection, and strict semantic model coverage. Candidate run `33590118787` passed the complete configured matrix for commit `e9e7a50`: Validate, unsigned iOS scaffold, Android debug, Web export/CI-served full-path smoke, source-bound long soak, Pages deploy, and public full-path/reload smoke. This evidence excludes Safari/mobile-browser, native install, production signing, physical-device, and human-feel coverage; the post-e9 first-breach/schema-7/Terms/stage-capture delta requires its own remote chain.
