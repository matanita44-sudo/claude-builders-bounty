...
- The iOS preset uses `com.matan.infinidive`, targets iPhone, and wires exact-size RGB/no-alpha icons. A fresh reconciled-tree unsigned Xcode evidence export at `../../build/ios-iphone-current` passes structural identity/icon/placeholder checks; the checked-in Team ID remains blank and no Xcode compile/archive, signed install, or TestFlight upload is evidenced.
